...
const createButton = () => {
    for(let i=0; i < buttonTable.length; i++) {
        const WholeButtonsContainer = document.querySelector("#calculator-buttons-container");
        const buttonContainer = document.createElement("div");
        buttonContainer.setAttribute("class",`buttons-${i+1} button`);
        const buttonGenerated = document.createElement("button");
        buttonGenerated.setAttribute("class",`buttons-content-${i+1} button-content`);
    ...
    button.addEventListener("click", () => {
        ...
        if (isNumeric(buttonListened)) {
            ...
            if ( isNumeric(addedNumber[addedNumber.length-1])) {
                document.getElementById("calculator-screen-numbers").textContent= addedNumber[addedNumber.length-1]+totalNumber;
            }
            else {
